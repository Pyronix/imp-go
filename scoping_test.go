...
var testWhileScopeTests = []WhileScopeCase{
	{Block(Sequence(Sequence(Declaration("x", Number(0)), Declaration("y", Number(1))), While(Lesser(Variable("x"), Number(10)), Block(Sequence(Assignment("x", Plus(Variable("x"), Number(1))), Declaration("y", Bool(true))))))), ValueState{map[string]Value{"x": IntValue(10)}, map[string]Value{"y": IntValue(1)}}, TypeState{map[string]Type{"x": TypeInt}, map[string]Type{"y": TypeBool}}, "{x := 0; y := 1; while x < 10 {x = x + 1; y := true}}"},
}

func runTest(e Statement) (ValueState, TypeState, string) {
	var vs []map[string]Value
	var ts []map[string]Type
	e.Eval(vs)
	e.Check(ts)
	pretty := e.Pretty()
	return vs, ts, pretty
